...
const New = ()=>{
    ...
    const Validate = Yup.object({
        ...
        age:Yup.string().max(2).required('Enter age'),
        location:Yup.string().required('Enter location'),
        des:Yup.string().required('Enter Description')
    })
    ...
    <div className="output-container">
        <div className="card-output mt-4" key={action.id}>
            ...
            <div className="card-body">
                ...
                <Link to={"/userDetails/" + action.id} > <h5 className="card-title">{action.name}</h5></Link>
...
<div className="second-output">
    ...
    <UserDetails db={db} users={users} setUser={setUser}/>
    </div>    
    </div> 
    </div>
    </>
